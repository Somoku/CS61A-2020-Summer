...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        var = [score0, score1]
        gain = var[who] - last_score
        if gain > running_high:
            print(gain, "point(s)! That's the biggest gain yet for Player", who)
            return announce_highest(who, var[who], gain)
        else:
            return announce_highest(who, var[who], running_high)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    def averaged(*args):
        trial = [original_function(*args) for _ in range(trials_count)]
        return sum(trial) / trials_count
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    res = [average_roll(i, dice) for i in range(1, 11)]
