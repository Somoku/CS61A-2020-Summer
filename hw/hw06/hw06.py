...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            print("'You must add ${0} more funds.'".format(
                self.price - self.money))
        elif self.money > self.price:
            print("'Here is your {0} and ${1} change.'".format(
                self.goods, self.money - self.price))
            self.money = 0
            self.stock -= 1
        else:
            self.money = 0
            self.stock -= 1
            print("'Here is your {0}.'".format(self.goods))

    def restock(self, num):
        self.stock += num
        print("'Current {0} stock: {1}'".format(self.goods, self.stock))

    def add_funds(self, fund):
        if self.stock == 0:
            print(
                "'Inventory empty. Restocking required. Here is your ${0}.'".format(fund))
        else:
            self.money += fund
